...
func (a *API) generateGo() ([]byte, error) {
	...
	for _, group := range a.EndpointGroups {
		for _, endpoint := range group.endpoints {
			...
			handleParams(p, i, endpoint.QueryParams, queryParams)
			handleParams(p, i, endpoint.PathParams, pathParams)
...
func handleParams(p func(format string, a ...interface{}), i func(paths ...string), params []Param, pt paramsType) {
