...
func (f *tsGenFile) createAPIClient(group *EndpointGroup) {
	f.p("export class %sHttpApi%s {", group.Prefix, strings.ToUpper(f.api.Version))
	f.p("\tprivate readonly http: HttpClient = new HttpClient();")
	f.p("\tprivate readonly ROOT_PATH: string = '/api/%s/%s';", f.api.Version, group.Prefix)
	f.p("")
	for _, method := range group.endpoints {
		...
		if method.Request != nil {
			f.p("\t\t\tconst response = await this.http.%s(path, JSON.stringify(request));", strings.ToLower(method.Method))
		} else {
...
func (f *tsGenFile) getArgsAndPath(method *fullEndpoint) (funcArgs, path string) {
	...
	path, _, _ = strings.Cut(method.Path, "{")
